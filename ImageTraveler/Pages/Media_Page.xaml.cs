...
namespace ImageTraveler.Pages
{
    ...
    public partial class Media_Page : Page
    {
        ...
        public Media_Page(Main_Command main_Command)
        {
            ...
            //vlcLibDirectory = new DirectoryInfo(Path.Combine(currentDirectory.FullName, "libvlc", "win-x86")); // 或者根据你的 VLC 路径调整

            //// 初始化 VLC 控件
            //vlcControl.SourceProvider.CreatePlayer(vlcLibDirectory);

            //// 播放 RMVB 文件
            //vlcControl.SourceProvider.MediaPlayer.Play(new Uri(@"D:\\SeanWu\\Media\\魔戒3.rmvb"));
        }
